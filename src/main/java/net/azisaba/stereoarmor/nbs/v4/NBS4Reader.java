...
import net.azisaba.stereoarmor.util.ByteUtil;
import org.jetbrains.annotations.NotNull;
import net.azisaba.stereoarmor.nbs.InvalidNBSHeaderException;
...
import net.azisaba.stereoarmor.nbs.NBSReader;
...
public class NBS4Reader implements NBSReader {
    @Override
    public @NotNull List<Byte> getSupportedVersions() {
        return ByteUtil.listOf((byte) 4, (byte) 5);
    }

    @Override
    public @NotNull ByteOrder getFileByteOrder() {
    ...
    public NBS4Header initialVersionChecks(ByteBuffer buffer) {
        ...
        if (!getSupportedVersions().contains(version)) {
            throw new InvalidNBSHeaderException(String.format("The specified file (Version %s) does not use a supported NBS version.", version));
        }
    ...
    protected NBSInstrument[] readInstrumentDataEntries(ByteBuffer buffer) {
        int instrumentCount = Byte.toUnsignedInt(buffer.get());
